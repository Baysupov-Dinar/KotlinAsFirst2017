...
class Line private constructor(val b: Double, val angle: Double) {
    ...
    fun crossPoint(other: Line): Point {
        if (this.angle == Math.PI / 2)
            return Point(-this.b, (-this.b) * tan(other.angle) + other.b / cos(other.angle))
        if (other.angle == Math.PI / 2)
            return Point(-other.b, (-other.b) * tan(this.angle) + this.b / cos(this.angle))
        val x = -(this.b / cos(this.angle) -
                other.b / cos(other.angle)) / (tan(this.angle) - tan(other.angle))
...
fun lineBySegment(s: Segment): Line {
    if (s.end.x == s.begin.x) return Line(s.begin, Math.PI / 2)
